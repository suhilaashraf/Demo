...
def set_led_state(state):
    ...
    if response:
        return int.from_bytes(response, byteorder='little')  # Assuming response is a single byte representing LED state
    else:
        return None
...
def get_led_state(PARAM_PORT,PARAM_PIN):
    ...
    response = receive_response()
    if response:
        return int.from_bytes(response)  # Assuming response is a single byte representing LED state
    else:
        return None
...
class TestLED(unittest.TestCase):
    ...
    @unittest.skipIf(get_switch_state(0) == 1, "Skipping this test case")
